...
function startDraw() {
	...
	function progress() {
		if (drawFlag) {
			drawFlag = false;
			count++;
			drawArc.bind(this)(this.option.ss, this.option.ss + count * ff);
			if (count >= _ff) {
				clearInterval(timer);
				drawFlag = false;
			}
...
function splitArc(x, y, r, start, end) {
	...
	for (let i = 0; i < t; i++) {
		...
		ctx.beginPath();
		setStrokeStyle.bind(this)(color[0], color[1], color[2], 1);
...
function createPalette(colorStops) {
	...
	return {
		...
		pickColor: position => imageData.slice(position * 4, position * 4 + 3)
	}
}
